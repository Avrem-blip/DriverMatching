...
[TestFixture]
public class SimpleSortingAlgorithmTests
{
    private SimpleSortingAlgorithm _algorithm = null!;

    [SetUp]
    public void Setup()
    {
        _algorithm = new SimpleSortingAlgorithm();
    }
    ...
    [Test]
    public void FindClosestDrivers_CorrectDistanceCalculation()
    {
        var drivers = new List<Driver>
        {
            new Driver(1, 0, 0),
        ...
        var result = _algorithm.FindClosestDrivers(drivers, order, 2);

        Assert.That(result[0].Distance, Is.EqualTo(0));
        Assert.That(result[1].Distance, Is.EqualTo(5)); // 3-4-5 triangle
    }
